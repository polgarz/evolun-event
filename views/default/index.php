    ...
    <?php if (Yii::$app->user->can('manageEvents')): ?>
        <div class="box-header">
            <div class="box-tools pull-left">
                <?= Html::a('<i class="fa fa-plus"></i> Új esemény', ['create'], ['class' => 'btn btn-success']) ?>
            </div>
        </div>
    <?php endif ?>
                                ...
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        ...
                                        <div class="text-muted">
                                            {participates} résztvevő &bullet; {start} &bullet; {place}
                                        </div>
                                    </div>
                                </div>
                            </a>
                ...
                [
                    ...
                    'buttons' => [
                        'gcalendar' => function($url, $model) {
                            $url = strtr('https://www.google.com/calendar/render?action=TEMPLATE&text={title}&dates={start}/{end}&details={description}&location={place}&sf=true&output=xml', [
                                '{title}' => urlencode($model->title),
                                '{place}' => urlencode($model->place),
                                '{description}' => urlencode($model->description),
                                '{start}' => date('Ymd\\THi00\\Z', strtotime($model->start . ' Europe/Budapest')),
                                '{end}' => $model->end ? date('Ymd\\THi00\\Z', strtotime($model->end . ' Europe/Budapest')) : null,
                            ]);
                            return Html::a('<i class="fa fa-calendar-plus-o"></i><span class="hidden-xs hidden-sm hidden-md"> Naptárhoz adom</span>', $url, ['class' => 'btn btn-default btn-sm', 'target' => '_blank']);
                        },
                    ],
                ],
            ],
